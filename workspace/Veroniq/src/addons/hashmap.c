...
hashmap_create(unsigned int nbuckets)
{
        ...
        ptr = /*safe*/calloc(1, sizeof(struct hashmap_s));
        if (!ptr)
                return NULL;

        ptr->size = nbuckets;
        ptr->buckets = /*safe*/calloc(nbuckets, sizeof(struct hashbucket_s));
        if (!ptr->buckets) {
                /*safe*/free(ptr);
                return NULL;
        }
...
hashmap_insert(hashmap_t map, const char *key, const void *data, size_t len)
{
         ...
         * problem later.
         */
        key_copy = /*safe*/strdup(key);
        if (!key_copy)
                return -ENOMEM;

        if (data) {
                data_copy = /*safe*/malloc(len);
                if (!data_copy) {
                        /*safe*/free(key_copy);
                        return -ENOMEM;
                }
        ...
        ptr = /*safe*/malloc(sizeof(struct hashentry_s));
        if (!ptr) {
                /*safe*/free(key_copy);
                /*safe*/free(data_copy);
                return -ENOMEM;
